/*  CoursesSection.jsx – NOV 17, 2025 | ALL CARDS SAME SIZE + Monthly Course Perfect */
import { motion, useMotionValue, useTransform, useSpring, useScroll } from "framer-motion";
import {
  BookOpen, Clock, Zap, ChevronRight, Star, Award, Users, Trophy,
...
const initialCourses = [
  {
    ...
    duration: "18 Days",
    description: "Complete iPhone Logic Board Repair Training by Sirajul Haque (for working technicians).",
    level: "Expert",
    students: "250+",
    rating: 4.9,
    gradient: "from-[#F37021] to-red-600",
    icon: <BookOpen className="w-7 h-7 sm:w-8 sm:h-8" />,
    features: ["Live iPhone Teardowns", "Realtime handskill development", "1-on-1 Mentorship"],
    badge: "BEST SELLER",
  ...
  },
  {
    ...
    duration: "3 Months",
    description: "Complete iPhone Basic–Advanced Course for Chip-Level Technicians.",
    level: "Advanced",
    students: "150+",
    rating: 4.8,
    gradient: "from-[#F37021] to-orange-700",
    icon: <Zap className="w-7 h-7 sm:w-8 sm:h-8" />,
    features: ["Live & Recorded Classes", "Website & App Access (watch anytime)", "Multi-Brand Repair"],
    badge: "FAST TRACK",
  ...
  },
  {
    ...
    duration: "12 Months",
    description: "Complete iPhone and Android Basic–Advanced Course for Chip-Level Technicians",
    level: "Expert",
    students: "250+",
    rating: 5.0,
    gradient: "from-red-600 to-purple-700",
    icon: <Award className="w-7 h-7 sm:w-8 sm:h-8" />,
    features: ["Recorded HD Classes & Live Q&A",  "Website & App Access", "Schematic Reading"],
    badge: "ELITE",
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14 sm:mb-16">
          <motion.h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black bg-clip-text text-transparent bg-gradient-to-r from-[#F37021] via-red-600 to-orange-700 mb-4 leading-tight"
            initial="hidden" whileInView="visible" viewport={{ once: true }}
            variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.08 } } }}>
            {"Learn With Us".split("").map((c, i) => (
              <motion.span key={i} variants={{ hidden: { opacity: 0, y: 60, rotateX: -60 }, visible: { opacity: 1, y: 0, rotateX: 0 } }}
                className="inline-block" style={{ transform: "translateZ(50px)" }}>
                {c === " " ? "\u00A0" : c}
              </motion.span>
            ))}
          </motion.h2>
          ...
          <div className="flex flex-wrap justify-center gap-8 sm:gap-12 mt-10">
            {[{ icon: <Users className="w-7 h-7" />, value: "700+", label: "Students" },
              ...
              { icon: <Star className="w-7 h-7" />, value: "4.9", label: "Rating" }].map((s, i) => (
        ...
        {/* COURSE GRID – ALL CARDS SAME HEIGHT */}
        ...
        {/* FINAL CTA */}
        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.8 }} className="mt-20 text-center">
          <motion.a href="https://wa.me/+918304952266" target="_blank" rel="noopener noreferrer"
            whileHover={{ scale: 1.06, boxShadow: "0 25px 50px rgba(243, 112, 33, 0.4)" }} whileTap={{ scale: 0.95 }}
...
// === COURSE CARD – FIXED HEIGHT & PERFECT ALIGNMENT ===
const CourseCard = ({
  id, name, duration, description, gradient, icon, features, badge, badgeColor, live, price, originalPrice, image, index, isDesktop, isMonthly
}) => {
  ...
  const cardContent = (
    <motion.div
      ...
      className="relative bg-white/80 backdrop-blur-2xl rounded-3xl overflow-hidden shadow-xl border border-white/50 hover:shadow-2xl transition-all duration-500 min-h-[680px] flex flex-col"
    >
      {/* Gradient overlay */}
      ...
      {isMonthly && (
        <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} viewport={{ once: true }}
          className="absolute top-4 left-4 z-20 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold text-xs px-3 py-1.5 rounded-full shadow-lg"
          style={isDesktop ? { transform: "translateZ(80px)" } : {}}
        >
          PER MONTH
        </motion.div>
      ...
      <div className="p-6 flex flex-col flex-grow justify-between space-y-5" style={isDesktop ? { transform: "translateZ(60px)" } : {}}>
        <div className="space-y-4">
          ...
          {isMonthly && <p className="text-xs text-center text-emerald-600 font-bold">12 Monthly Payments</p>}
          ...
          <div className="text-center space-y-2">
            <div className="flex items-center justify-center gap-2 flex-wrap">
              ...
              {isMonthly && <span className="text-sm font-medium text-gray-600">/month</span>}
              {originalPrice && (
          ...
          <div className="flex justify-center items-center gap-1">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className={`w-4 h-4 ${i < 4.5 ? "text-yellow-500 fill-current" : "text-gray-300"}`} />
            ))}
      ...
      {/* Shine effect */}
      <motion.div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/60 to-transparent opacity-0 group-hover:opacity-100 pointer-events-none"
        initial={{ x: "-150%" }} whileHover={{ x: "150%" }} transition={{ duration: 0.9 }} />
    </motion.div>
  );
